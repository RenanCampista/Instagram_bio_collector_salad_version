    ...
    def send_json(self, data: dict) -> bool:
        ...
        try:
            response = requests.post(
                self.route, json=data, headers=self.headers, timeout=30000
            )
            if response.status_code == 200:
                ...
                self.log.error(
                    f"Erro ao enviar os dados: {response.status_code} - {response.text}"
                )
                return False
        except requests.exceptions.Timeout:
            self.log.error(
                "Timeout ao enviar os dados. A API demorou mais de 5 minutos para responder."
            )
            return False
