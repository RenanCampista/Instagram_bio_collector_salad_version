...
class InstagramProfileFetcher:
    ...
    def check_rate_limit_in_output(
        self, error_message: str, captured_output: str = ""
    ) -> bool:
        ...
        return any(
            indicator.lower() in full_text for indicator in rate_limit_indicators
        )
    ...
    def fetch_profile(self, username: str) -> tuple[dict | None, str, int]:
        ...
        try:
            ...
            self.log.warning(
                f"Erro de conexão ou requisição para o perfil {username}: {e}"
            )
